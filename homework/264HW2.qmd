...
```{r}
spot_smaller |>
  mutate(num_title_words = str_count(str_remove(title, " \\(feat.*| \\(with.*"), " ") + 1)
```
...
first_word_vec <- spot_smaller$first_word
first_word_vec
```
...
```{r}
bigspotify |>
  mutate(has_decade = str_detect(playlist_name, "\\d0's|\\d0s"),
         playlist_decade = ifelse(has_decade == TRUE, parse_number(str_extract(playlist_name, "\\d0's|\\d0s")), NA)) |>
